...
class AnyLinePreprocessor:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("IMAGE",),
                "merge_with_lineart": (["lineart_standard", "lineart_realisitic", "lineart_anime", "manga_line"],
    ...
    def get_anyline(self, image, merge_with_lineart, resolution, lineart_lower_bound=0, lineart_upper_bound=1,
        ...
        mteed_model = TEDDetector.from_pretrained("TheMistoAI/MistoLine", "MTEED.pth", subfolder="Anyline").to(
            self.device)
        ...
        if merge_with_lineart == "lineart_standard":
            ...
            lineart_result = common_annotator_call(lineart_standard_detector, image, guassian_sigma=2,
                                                   intensity_threshold=3, resolution=resolution,
                                                   show_pbar=False).numpy()
            ...
            lineart_detector = dict(lineart_realisitic=LineartDetector, lineart_anime=LineartAnimeDetector,
                                    manga_line=LineartMangaDetector)[merge_with_lineart]
            lineart_detector = lineart_detector.from_pretrained().to(self.device)
            lineart_result = common_annotator_call(lineart_detector, image, resolution=resolution,
                                                   show_pbar=False).numpy()
        ...
        for i in range(len(image)):
            _lineart_result = get_intensity_mask(lineart_result[i], lower_bound=lineart_lower_bound,
                                                 upper_bound=lineart_upper_bound)
            _cleaned = morphology.remove_small_objects(_lineart_result.astype(bool), min_size=object_min_size,
